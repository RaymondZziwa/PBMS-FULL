...
//route to fetch client associated receipts
app.post('/fetchclientassociatedreceipts', (req, res) => {
    jwt.verify(req.body.token, 'SECRETKEY', (err) => {
        if (err) {
            res.status(403).send("You are not authorized to perform this action.");
        } else {
            const name = req.body.clientName
            db.query('SELECT receiptNumber, saleDate, customerNames, itemsSold, totalAmount, paymentStatus FROM equatorialShopSales WHERE customerNames = ?', [name] ,(error, results) => {
                if (results.length > 0) {
                    res.send(results)
                } else {
                    res.send(`There are no records found.`)
...
app.get('/testitems', (req, res)=> {
    res.send({name: 'name: test route', items: ['item1', 'item2']});
})
