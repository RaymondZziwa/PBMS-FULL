const shopStoreModel = require('../models/productsInventoryModel')

const moveStock = async (req, res) => {
    const {date, branch, category, items, source, notes} = req.body;
    try {
        const newStockLevels = await shopStoreModel.productStockMvtModel(date, branch, category, items, source, notes)
...
const stockTaking = async (req, res) => {
    try {
        ...
        const data = await shopStoreModel.stockTakingModel(branch)
        if(!data.length){
...
const fetchStockMovementRecords = async (req, res) => {
    try {
        const {branch} = req.body
        const data = await shopStoreModel.fetchAllTransactionsModel(branch)
