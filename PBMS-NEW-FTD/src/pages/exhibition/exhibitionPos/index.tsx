...
import useExhibitionStoreInventory from '../../../hooks/exhibitions/useExhibitionStoreInventory';
import useExhibitionStore from '../../../hooks/exhibitions/useExhibitionStore';
...
const ExhibitionPOS: React.FC = () => {
  const {data: stores} = useExhibitionStore()
  const store = JSON.parse(localStorage.getItem('posStore') || '{}');
  const {data: allItems, refresh: getStockLevels} = useExhibitionStoreInventory(store.storeId);
  ...
  if (!selectedStore) {
    return (
      <StoreSelectionModal
        ...
        stores={(stores || []).filter(store => new Date(store.exhibition.endDate) >= new Date())}
