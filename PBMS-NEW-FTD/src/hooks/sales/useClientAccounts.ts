...
import { fetchDataStart, fetchDataSuccess, fetchDataFailure } from "../../redux/slices/sales/clientAccountSlice";
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest(
          SALESENDPOINTS.CLIENT_ACCOUNTS.find_all,
          "GET",
            '',
      );
...
const data = useSelector((state: RootState) => state.clientAccount);
